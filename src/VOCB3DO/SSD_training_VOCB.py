...
callbacks = [keras.callbacks.ModelCheckpoint('../checkpoints/VOCB3DO/weights-v6.{epoch:02d}-{val_loss:.2f}.hdf5',
                                             verbose=1,
                                             save_best_only=True,
...
nb_epoch = 200
history = model.fit_generator(gen.generate(True), num_train,
                              nb_epoch, verbose=1,
                              callbacks=callbacks,
...
model.load_weights('./checkpoints/weights.14-2.02.hdf5', by_name=True)
inputs = []
images = []
img_path = path_prefix + sorted(val_keys)[34]
