        ...
        while True:
            ...
            for rgb_key, depth_key in zip(rgb_keys, depth_keys):            
                ...
                if len(targets) == self.batch_size:
                    ...
                    inputs = []
                    targets = []
                    tmp_depth_inp = preprocess_input(np.transpose(tmp_depth_inp, (0, 3, 1, 2))).transpose(0, 2, 3, 1)
                    yield [np.array(preprocess_input(tmp_rgb_inp)), np.array(tmp_depth_inp)], [tmp_targets]
    ...
    while True:
        ...
        if iter_num == epoch_length:
            ...
            if curr_loss < best_loss:
                ...
                best_loss = curr_loss
                model.save_weights('/media/jun/checkpoints/SUNRGBD/RGBD/v2-1/weights-v2-1.{epoch:02d}-{val_loss:.2f}.hdf5'.format(epoch=epoch, val_loss=best_loss))
            iter_num = 0
            break
