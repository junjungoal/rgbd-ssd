...
depth_input_shape = (300, 300, 3) #channel last
...
class Generator(object):
    def __init__(self, rgb_gt, depth_gt, bbox_util,
                 ...
                 rgb_train_keys, depth_train_keys, rgb_val_keys, depth_val_keys, image_size,
                 saturation_var=0.5,
                 brightness_var=0.5,
                 contrast_var=0.5,
                 lighting_std=0.5,
    ...
    def generate(self, train=True):
        while True:
            ...
            for rgb_key, depth_key in zip(rgb_keys, depth_keys):            
                ...
                depth_img = cv2.imread(depth_img_path, cv2.IMREAD_COLOR).astype('float32')
                depth_img = cv2.applyColorMap(depth_img, cv2.COLORMAP_JET)
                y = self.rgb_gt[rgb_key].copy()
                if train and self.do_crop:
                    rgb_img, y = self.random_sized_crop(rgb_img, y)
                    depth_img, y = self.random_sized_crop(depth_img, y)
                rgb_img = imresize(rgb_img, self.image_size).astype('float32')
                depth_img = imresize(depth_img, self.image_size).astype('float32')
                if train:
                    shuffle(self.color_jitter)
                    for jitter in self.color_jitter:
                        rgb_img = jitter(rgb_img)
                        depth_img = jitter(depth_img)
                    if self.lighting_std:
                        rgb_img = self.lighting(rgb_img)
                        depth_img = self.lighting(depth_img)
                    if self.hflip_prob > 0:
                        rgb_img, y = self.horizontal_flip(rgb_img, y)
                        depth_img, y = self.horizontal_flip(depth_img, y)
                    if self.vflip_prob > 0:
                        rgb_img, y = self.vertical_flip(rgb_img, y)
                        depth_img, y = self.vertical_flip(depth_img, y)
                ...
                if len(targets) == self.batch_size:
                    ...
                    inputs = []
                    targets = []
                    yield [np.array(preprocess_input(tmp_rgb_inp)), np.array(preprocess_input(tmp_depth_inp))], [tmp_targets]


path_prefix = '../dataset/'
gen = Generator(rgb_gt, depth_gt, bbox_util, 16, path_prefix,
                rgb_train_keys,depth_train_keys, rgb_val_keys, depth_val_keys,
                (rgb_input_shape[0], rgb_input_shape[1]), do_crop=True)
    ...
    while True:
        ...
        if iter_num == epoch_length:
            ...
            if curr_loss < best_loss:
                ...
                best_loss = curr_loss
                model.save_weights('../checkpoints/SUNRGBD/RGBD/v3/weights-v3.{epoch:02d}-{val_loss:.2f}.hdf5'.format(epoch=epoch, val_loss=best_loss))
            iter_num = 0
            break
